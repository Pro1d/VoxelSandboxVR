class_name EditorBoxDraw
extends EditorBase

const MAX_BOX_VOLUME := 512 # cells^3 ; 512 = 8x8x8 or 512x1x1 or 64x4x2 or ...
...
func _clamped_box_end_index() -> Vector3i:
	...
	if desired_volume > MAX_BOX_VOLUME:
		var allowed_size := Vector3(desired_size) * pow(float(MAX_BOX_VOLUME) / desired_volume, 1.0 / 3)
